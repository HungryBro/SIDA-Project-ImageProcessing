...
if ($condaPath) {
    ...
    if (Test-Path $envPython) {
        ...
        & $envPython -m pip install -r (Join-Path $PSScriptRoot "requirements.txt")
        Write-Host "[+] Environment setup successfully using Conda!" -ForegroundColor Green
        Write-Host "[*] To run your script, use: conda activate $envName; python gradcam_yolo.py" -ForegroundColor Yellow
    } else {
...
elseif ($pythonPath) {
    ...
    if (Test-Path $venvPython) {
        ...
        Write-Host "[*] To run your script, activate with: .venv\Scripts\Activate.ps1; python gradcam_yolo.py" -ForegroundColor Yellow
    } else {
